...
public interface ProfilePassionRepository extends JpaRepository<ProfilePassion, Integer> {
    ...
    @Query(
            "SELECT p1.profileId AS MatchProfileId, COUNT(p1.passionId) AS SimilarityScore FROM"
                ...
                + " true AND m.like_2 = true) OR (m.like_1 = false AND m.like_2 = false)) OR"
                + " (m.profileId_1 = p2.profileId AND m.profileId_2 = p1.profileId)) GROUP BY"
                + " p1.profileId ORDER BY SimilarityScore DESC, MatchProfileId ASC")
